...
//			int predCount=0;
			for(Sentence sen:testCorpus){
				for(Predicate pred:sen.getPredicates()){
					...
					// for subsequent sentences, use the (local) highest scoring argmap 
	...
	private boolean equal(ArgMap newargs, Map<Word, String> oldargs) {
		for(Word w : newargs.keySet()) {
			if(!oldargs.containsKey(w) || !oldargs.get(w).equals(newargs.get(w)))
				return false;
		}
...
//			sumProbs+=prob;
    ...
    private void addAndCollectGlobalFeatures(Predicate pred,ArgMap argMap,Collection<Integer> indices, Map<Integer, Double> nonbinFeats) {
		...
		int argsize = argMap.size();
		for(int i=0; i<argsize; i++) {
			Integer[] span1 = DasFilter.pass(pred, (Word)argMap.keySet().toArray()[i]);
			if(span1==null) continue;
			for(int j=i+1; j<argsize; j++) {
				Integer[] span2 = DasFilter.pass(pred, (Word)argMap.keySet().toArray()[j]);
				if(span2==null) continue;
				if(span1[0]<span2[1] && span1[1]>span2[1])
					conflict=true;
				if(span2[0]<span1[1] && span2[1]>span1[1])
					conflict=true;
			}
		}
		if(conflict) indices.add(offset+sizePipelineFeatures);
		offset+=1;*/
