...
public class Pipeline extends SemanticRoleLabeler {
	...
	public static Pipeline trainNewPipeline(Iterable<Sentence> sentences,
			FeatureGenerator fg, ZipOutputStream zos,
			Map<Step, FeatureSet> featureSets) throws IOException {
		// Then create the arglabel list and predicatereference
		Pipeline pipeline = new Pipeline(fg, featureSets, sentences);
		// pipeline.extractLabels(sentences,
		// featureSets.get(Step.pd).POSPrefixes);
		...
		if (zos != null) {
			...
			for (Step s : Step.values()) {
				zos.putNextEntry(new ZipEntry(LearnOptions.featureFileNames
						.get(s)));
				FeatureFile.writeToOutput(featureSets.get(s), new PrintWriter(
						zos));
				zos.closeEntry();
			}
			// Then write the predicatereference, the arglabel list and the
			// featuregenerator.
	...
	public static Pipeline trainNewPipeline(Iterable<Sentence> sentences,
			Map<Step, File> featureFiles, ZipOutputStream zos, BrownCluster bc,
			WordEmbedding we) throws IOException {
		// Start by creating the feature mappings for all features based on the
		// feature files.
