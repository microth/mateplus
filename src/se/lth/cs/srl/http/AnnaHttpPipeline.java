...
public class AnnaHttpPipeline extends AbstractPipeline {
	...
	public String getParseInterfaceHTML(L l) {
		String s = "  <h3>Try the parsing pipeline</h3>\n"
	...
	public StringPair parseRequest(String inputSentence,
		...
		if (vars.containsKey("returnType")
			...
			boolean includeDependencyGraphImage = vars
					.containsKey("doRenderDependencyGraph") && pp.hasParser();
	...
	private String getHTMLResponse(Sentence sen, long time,
		...
		for (String line : sen.toString().split("\n")) {
			...
			for (String token : line.split("\t")) {
				if (token == "null" || tokIdx % 2 == 0 && tokIdx > 1
						&& tokIdx < 12)
					token = "_";
	...
	public static void main(String[] args) throws IOException {
		...
		AnnaHttpPipeline ahp = new AnnaHttpPipeline(pp,
