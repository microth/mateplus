...
public class ParseRequestHandler extends AbstractHandler {
	...
	public ParseRequestHandler(DefaultHandler defaultHandler,
			AbstractPipeline pipeline) throws IOException {
		super(pipeline);
		this.defaultHandler = defaultHandler;
	}

	private static final Pattern WHITESPACE_ONLY_PATTERN = Pattern
			.compile("^\\s+$");
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		Map<String, String> vars = AbstractHandler
				.contentToVariableMap(getContent(exchange));
		...
		String xForwardedFor = AbstractHandler.getHeader(exchange,
				"X-forwarded-for"); // Header provided by apache mod_proxy

		// Parse
		String httpResponse;
		String content_type;
		System.out.println("@Parsing ``" + inputSentence + "'' at "
				+ new Date(System.currentTimeMillis()));
		System.out.println("Requested by "
				+ exchange.getRemoteAddress()
				+ (xForwardedFor == null ? "" : " (Forwarded for "
						+ xForwardedFor + ")") + ". Agent: " + userAgent);
		...
		try {
			...
			sendContent(exchange, "Sentence too long (" + e.length
					+ " tokens). Request denied.", "text/plain", 413);
			System.out.println("Aborting, sentence too long: " + e.length
					+ " tokens.");
			return;
		} catch (Throwable t) {
			t.printStackTrace();
