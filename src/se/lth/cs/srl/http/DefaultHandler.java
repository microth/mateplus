...
public class DefaultHandler extends AbstractHandler {
	...
	public DefaultHandler(L currentL, AbstractPipeline pipeline) {
		super(pipeline);
		setupPages(currentL); // Note this is a bit nasty, with the pages being
								// static, this being an object setting the
								// language stuff, but we want the pages to be
								// static. Or rather, this is TODO: make the
								// handler non-static and use the object
								// instead.
	...
	private void servePage(String pageName, HttpExchange exchange)
			throws IOException {
		getContent(exchange); // We dont care about this, but need to read it to
								// be on the safe side
		String content;
		if (pageName.equals("default")) {
			content = HTML_HEAD + pipeline.HTML_TOP_EXTRA + pages.get(pageName)
					+ pipeline.HTML_BOTTOM_EXTRA + HTML_TAIL;
