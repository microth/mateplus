...
public class CorefFeature extends SingleFeature {
	...
	protected CorefFeature(boolean full, String POSPrefix) {
		super(full?FeatureName.FullCoref:FeatureName.XNCoref,true,false,POSPrefix);
		this.full = full;
	...
	public String getFeatureString(Predicate pred, Word arg) {
		...
		String cc = s.getMyCorpus().findChainInPrvSentence(s.getMyCorpus().corefId(arg), s, true);

		//if(!cc.equals("_") && pred.getArgMap().get(arg)!=null) {
		//	System.out.print("Corpus " + s.getMyCorpus().getName() + " / Sentence " + s.getMyCorpus().indexOf(s)+ " ");
		//	System.out.println(pred.getSense()+"["+pred.getArgMap().get(arg)+"]: " + arg.getForm() + "(previous sentence: "+cc+")");
		//}
