...
public abstract class WordExtractor implements Serializable {
	...
	public static WordExtractor getExtractor(TargetWord tw){
		...
		case FirstWord:		return new FirstWord();
		case SecondWord:    return new SecondWord();
		case LastWord:		return new LastWord();
	...
	private static class SecondWord extends WordExtractor {
		private static final long serialVersionUID = 1L;
		public Word getWord(Sentence s, int predIndex, int argIndex) {
			return s.get(argIndex).getSpan().size()==1?null:(Word)s.get(argIndex).getSpan().toArray()[1];
		}
		public Word getWord(Word pred,Word arg){
			return arg.getSpan().size()==1?null:(Word)arg.getSpan().toArray()[1];
		}
	}
	private static class LastWord extends WordExtractor {
		private static final long serialVersionUID = 1L;
		public Word getWord(Sentence s, int predIndex, int argIndex) {
			return (Word)s.get(argIndex).getSpan().toArray()[s.get(argIndex).getSpan().size()-1];
		}
		public Word getWord(Word pred,Word arg){
