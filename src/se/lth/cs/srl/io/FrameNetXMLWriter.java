...
public class FrameNetXMLWriter implements SentenceWriter {
	...
	public FrameNetXMLWriter(File filename) {
	    ...
	    try {
	    	...
	    	out.write("<corpus ID=\"100\" XMLCreated=\""+new Date() +"\" name=\"ONE\">\n");
	    	out.write("  <documents>\n");
        ...
        try {
			...
			for(int i=1; i<s.size(); i++) {
				if(s.get(i) instanceof Predicate) {
					...
					out.write("              <annotationSet ID=\""+annID+"\" frameName=\""+p.getSense()+"\">\n");
					out.write("                <layers>\n");
					...
					out.write("                  <layer ID=\""+layerID+"\" name=\"FE\">\n");
					out.write("                    <labels>\n");
					
					if(!reconstructSpan)
						for(Word a :  p.getArgMap().keySet()) {
							out.write("                      <label ID=\""+labelID+"\" end=\""+ (a.getEnd()-1) +"\" name=\"" + p.getArgMap().get(a) + "\" start=\"" + a.getBegin() +"\"/>\n");
							labelID++;
						} 
					else {
						...
						for(Word a : p.getArgMap().keySet()) {
							...
							if(begin_end==null)	continue;
							
							String label = p.getArgMap().get(a);
							if(!label2spans.containsKey(label))
								label2spans.put(label, new LinkedList<Word[]>());
						...
						for(String label : label2spans.keySet()) {
							List<Word[]> begin_ends = DasFilter.merge(label2spans.get(label));
						...
						for(String label : newlabel2spans.keySet()) {
							...
							for(Word[] begin_end : begin_ends) {
								out.write("                      <label ID=\""+labelID+"\" end=\""+ (begin_end[1].getEnd()-1) +"\" name=\"" + label + "\" start=\"" + (begin_end[0].getBegin()) +"\"/>\n");
								labelID++;
							}
						}
