        		...
        		if(p.getSense().equals("Action") || p.getSense().equals("OPERATION")) {
            		...
            		for(Word w : p.getArgMap().keySet()) {
            			...
            			out.write("R" + (rnum++) + "\t" +
            					...
            					(label.equals("Property")?("HasProperty Arg1:"+id(p)+ " Arg2:"+id(w)):
            										  ("ActsOn Arg1:"+id(p)+ " Arg2:"+id(w)))) +
            				     "\n");
            		}
        		}
        			
        		if(p.getSense().equals("Object") || p.getSense().equals("CONCEPT") || p.getSense().equals("Property")) {
        			if(!word2id.containsKey(p))
        				out.write(id(p) + "\t" + p.getSense() + " " + p.getBegin() + " " + p.getEnd() + "\t" + p.getForm()+"\n");
