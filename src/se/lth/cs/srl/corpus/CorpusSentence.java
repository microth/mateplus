...
public class CorpusSentence extends Sentence {
	...
	public String toSpecialString() {
		...
		for (int i = 1; i < super.size(); ++i) {
			...
			// coref ids: a bit of a hack now--they should be stored for each
			// word instead
	...
	public int findhead(int begin, int end) {
		...
		if (this.get(currids.get(0)).getPOS().startsWith("V"))
			return currids.get(0);
		if (this.get(currids.get(0)).getPOS().startsWith("J"))
		 ...
		 * this.get(head).getHeadId(); containshead = false; for(int i=0;
