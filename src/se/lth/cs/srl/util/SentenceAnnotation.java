...
public class SentenceAnnotation {
		...
		if(num_anno_word.length==3) {
			if(id2conceptAnno.containsKey(num_anno_word[0]) && num_anno_word[0].startsWith("F")) {
				...
				if(to1+1==from2) {
					...
					anno[1] = name_from_to1[0] + " " + from1 + " " + to1 + " " + from2 + " " + to2;
				//anno[1] = name_from_to1[0] + " " + (from1<from2?from1:from2) + " " + (to1>to2?to1:to2);
	...
	public void apply(Sentence sen, boolean framenet) {
		...
		for(String[] anno : conceptAnno) {
			...
			for(int i=1; i+1<parts.length; i+=2) {
				...
				if(currids.size()==0) {
					...
					if(!framenet) {
						...
						for(int j=0; j<begins.length;j++) {
							System.err.println("  " + begins[j] + ":" + ends[j] + "\t" + sen.get(j+1).getForm());
						}
						continue;
					}
					for(int j=0; j<begins.length;j++) {
						if(begins[j]>=startCharacter && endCharacter<=ends[j]) {
							System.err.println("Mapped to first surrounding token instead ("+begins[j] + ":" + ends[j] +")");
							currids.add(j);
							break;
						}
					}				
			...
			concept2spans.get(anno[0]).add(currids.toArray(new Integer[currids.size()]));
		...
		for(String anno : concept2spans.keySet()) {
			...
			if(framenet && anno.startsWith("F")) {
				...
				//System.err.print("Frame ("+anno[1].split(" ")[0]+"): " + sen.get(head(sen, currids)+1).getForm());
			} else {
				for(Integer[] ids : concept2spans.get(anno)) {
					...
					if(ws.length==1) {
						...
						if(label.equals("Object") && (w.getPOS().equals("TO") || w.getPOS().equals("IN")) && !w.getChildren().isEmpty())
							concept2word.put(anno, w.getChildren().iterator().next());
					} else {
					...
					System.err.print(anno[0]+" (deps):");				
					int head = head(sen, currids);
					if(last_target==head) {
						int i = head;
						System.err.print((head==i?"(":"") + (last_target==i?"*":"") + sen.get(i+1).getForm() + (last_target==i?"*":"") + (head==i?")":""));
						System.err.println("\n---");
						continue;
					}
					...
					while(!todo.isEmpty()) {
						...
						for(Word w : sen.get(curr+1).getChildren()) {
							...
							if(done.contains(child))
								continue;
							**/
							/*if(child<min && head<last_target) {
								min = child;
								todo.add(child);
							}
							if(child>max && head>last_target) {
								max = child;
								todo.add(child);
							}*/
					/**
							if(!contains(sen, child, last_target)) {
								todo.add(child);
								if(child<min && (min<last_target || (min>last_target && child>last_target))) min = child;
								if(child>max && (max>last_target || (max<last_target && child<last_target))) max = child;
							}
						}
					}
		...
		for(String[] anno : relationAnno) {
			...
			if(framenet) {
				...
				if(concept2word.get(arg1)==null || concept2word.get(arg2)==null) continue; // HACK
	...
	private int[] heads(Sentence sen, List<Integer> currids) {
		if(currids.size()==1) return new int[]{currids.get(0)};		
		if(sen.get(currids.get(0)+1).getPOS().startsWith("V")) return new int[]{currids.get(0)};
		...
		if(containsSubtrees) {
			if(sen.get(currids.get(0)+1).getPOS().startsWith("J")) return new int[]{currids.get(currids.size()-1)};
	...
	public int head(Sentence sen, List<Integer> currids) {
		...
		if(sen.get(currids.get(0)+1).getPOS().startsWith("V")) return currids.get(0);
		if(sen.get(currids.get(0)+1).getPOS().startsWith("J")) return currids.get(currids.size()-1);
		if(sen.get(currids.get(0)+1).getPOS().startsWith("N")) {
			for(Integer i : currids) {
				if(sen.get(i+1).getForm().equals("of"))
