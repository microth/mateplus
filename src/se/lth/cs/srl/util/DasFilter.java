...
public class DasFilter {
	...
	public static Word[] pass(Predicate p, Word a) {
		...
		if(p.getIdx()==a.getIdx()) return new Word[]{a,a};
		
		// filter from Hermann et al. (2014)
		/*if(p.getHead().getChildren().contains(a)) {				
			Word leftmost = a;
			Word rightmost = a;
			while(leftmost.getLeftmostDep()!=null) leftmost = leftmost.getLeftmostDep();
			while(rightmost.getRightmostDep()!=null) rightmost = rightmost.getRightmostDep();
		...
		// filter from Xue and Palmer (2004)
		for(Word w : Word.pathToRoot(p, new LinkedList<Word>())) {			
			if(w.getChildren().contains(a)) {
				Word leftmost = a;
				Word rightmost = a;
				while(leftmost.getLeftmostDep()!=null) leftmost = leftmost.getLeftmostDep();
				while(rightmost.getRightmostDep()!=null) rightmost = rightmost.getRightmostDep();
		...
		if(a.getIdx()>p.getIdx() && leftmost.getIdx()<=p.getIdx()) {
			//leftmost = rightmost = a; //->Recall=0.64146 (6668.0/10395.0)  Precision=0.68481 (6668.0/9737.0)  Fscore=0.66243
			if(p.getPOS().startsWith("V"))
				return null; //-> Recall=0.62217 (6467.5/10395.0)  Precision=0.72178 (6467.5/8960.5)  Fscore=0.66829
			
			leftmost = p.getMySentence().get(p.getIdx()+1); //-> Recall=0.64767 (6732.5/10395.0)  Precision=0.69143 (6732.5/9737.0)  Fscore=0.66884
		}
	...
	public static List<Word[]> merge(List<Word[]> list) {
		...
		LIST: for(Word[] begin_end : list) {
			...
			for(Word[] existing : retval) {				
				if(begin_end[0].getIdx()>=existing[0].getIdx() && begin_end[1].getIdx()<=existing[1].getIdx()) {
					// completely surrounded
					add = false;
					break;
				}
				if(begin_end[0].getIdx()==existing[1].getIdx()+1 || (begin_end[0].getIdx()<existing[1].getIdx() && begin_end[1].getIdx()>existing[1].getIdx()) ) {
					existing[1] = begin_end[1]; // extend end
					add = false;
					break;
				}
				if(begin_end[1].getIdx()==existing[0].getIdx()-1 || (begin_end[1].getIdx()>existing[0].getIdx() && begin_end[0].getIdx()<existing[0].getIdx())) {
			...
			for(String label2 : newlabel2spans.keySet()) {
				...
				for(int i=0; i<size1; i++) {
					for (int j=0; j<size2; j++) { 
						...
						int end1 = spans1.get(i)[spans1.get(i).length-1].getIdx();
						int end2 = spans2.get(j)[spans2.get(j).length-1].getIdx();
						...
						if(change1) {
							...
							for(int x=oldspan[0].getIdx(); x<=oldspan[1].getIdx(); x++)
								System.out.print(s.get(x).getForm() + " ");
							System.out.print("->");
							for(int x=span[0].getIdx(); x<=span[1].getIdx(); x++)
								System.out.print(" " + s.get(x).getForm());
							System.out.println();
