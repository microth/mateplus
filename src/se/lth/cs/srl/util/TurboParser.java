	...
	public void overwriteParse(Sentence s) {
		try {
			...
			for(int i=1; i<s.size(); i++) {
				...
				// if current line is blank (end of last sentence), read next line
				if(line.equals("")) line = br.readLine();
				
				String[] parts = line.split("\t");				
				// sanity check
				if(!parts[1].toLowerCase().equals(w.getForm().toLowerCase())) {
					System.err.println("WARNING: different normalization applied? ("+parts[1] + " vs. " + w.getForm()+")");
				...
				// CoNLL-09
				/**w.setPOS(parts[4]);
				   w.setHeadId(Integer.parseInt(parts[8]));
